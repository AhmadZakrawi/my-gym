...
const Calendar = () => {
  ...
  const getAllClasses = async () => {
    ...
    if (cls) {
      cls.map(c => {
        ...
        return c.type === 'onetime' ?  allEvents.push(generateOnetimeEvents(c)) : 
  ...
  return (
    <>
    {getLoading || deleteLoading ? <div className="h-full w-full flex items-center justify-center"><CircularProgress size={80} /></div> : (
      <Paper>
      ...
      <Scheduler data={allClasses}>
        ...
        <MonthView startDayHour={6} endDayHour={23}/>
        <Appointments />
